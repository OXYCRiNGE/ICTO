...
for i in range(find_count_files(dirname)):
    ...
    path_to_pdf = f'{dirname}\{list_pdf[i-1]}'
    ...
    #     pages[0].save('img/out' + str(page) + '.jpg', 'JPEG')
    ...
    black_num = []
    fio = []
    for k in range(find_count_files('img')):
    ...
    df.to_csv(f'{new_name}\{new_name}{i}.csv',sep=';',  header=False, index=False)
